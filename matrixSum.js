...
function findMatrixSum(matrix) {
  ...
  var inner = function(visitedCols, sum, position) {
    if (position >= matrix.length) return sum;

    var arr = matrix[position];
    var total = 0;

    for (var i = 0; i < arr.length; ++i) {
      if (!visitedCols.includes(i)) {
        total = inner(visitedCols.concat([i]), sum + arr[i], position + 1);
      }
    }
